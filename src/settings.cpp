...
std::shared_ptr<settings> cg::settings::parse_settings(int argc, char** argv)
{
	...
	add_options("height", "Render target height", cxxopts::value<unsigned>()->default_value("2160"));
	add_options("width", "Render target width", cxxopts::value<unsigned>()->default_value("3840"));
	add_options("model_path", "Path to OBJ model", cxxopts::value<std::filesystem::path>()->default_value("models/z_test.obj"));
